...
""" #Much too small :(
# Estimate Number 1 for Error on Rate for all angles
m = 3       # Background outside of n*sigma
abw = []

for k in range(9):
    popt,pcov = curve_fit(gaussian,x[k],y[k],p0=paramg)
    mu,sigma = popt[1],popt[2]
    #imu,iunten,ioben = index(x[k],mu),index(x[k],mu-m*sigma),index(x[k],mu+m*sigma)
    i1,i2,i3,i4 = index(x[k],mu-m*sigma-1),index(x[k],mu-m*sigma),index(x[k],mu+m*sigma),index(x[k],mu+m*sigma),
    buf = 0
    n = 0
    for j in y[k][i1:i2]:
        buf += (j-bg[k])**2
    ...
    for j in y[k][i3:i4]:
        buf += (j-bg[k])**2
        n += 1
    abw.append(np.sqrt(buf/n))
"""


""" #Much too big :(
# Estimate Number 2 for Error on Rate for all angles
buf = 0
for k in range(9):
    buf += (nevents[k]-scaler[k])**2
abw = np.sqrt(buf/(9*8))
print(abw)
"""
